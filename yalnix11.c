...
void KernelStart(ExceptionStackFrame *frame, unsigned int pmem_size, void *org_brk, char **cmd_args)
{
	...
	*(int *)(long)(addr)=-1;
	...
	new_pcb->pt0_virt=pt0;//Get_Page_table(&(new_pcb->pt0_phy));
	new_pcb->pt0_phy=pt0;
	TracePrintf(0,"Returned from get page table\n");

	pcb_head=new_pcb;
	new_pcb=(struct PCB *)malloc (sizeof(struct PCB));
	new_pcb->pid=pid_counter++;
	new_pcb->pt0_virt=pt0;
	new_pcb->pt0_phy=pt0;
	...
	res=ContextSwitch(MySwitchFunc, &new_pcb->ctxp, (void *)pcb_head, (void *)pcb_head);
		if(res==-1)
		{
