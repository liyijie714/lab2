...
void kexit(int status)
{
	int i;
	struct pte *pt0;
	struct PCB *old_pcb,*child_pcb,*ptr;
	...
	if(old_pcb->parent!=NULL)
	{
		...
		ptr=wait_blockedQ.front;
		while(ptr)
		{	if(ptr==old_pcb->parent)
			{	remove_from_Q(&wait_blockedQ,ptr);
				active_pcb=ptr;
				break;
			}
			ptr=ptr->next;
