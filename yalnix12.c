...
long GetPage();
void ReturnPage(struct pte *p1,int index);
...
void KernelStart(ExceptionStackFrame *frame, unsigned int pmem_size, void *org_brk, char **cmd_args)
{
	...
	*(int *)(long)(addr)=-1;
	...
	init_pcb->pt0_virt=pt0;//
	init_pcb->pt0_phy=pt0;
	
	new_pcb=(struct PCB *)malloc (sizeof(struct PCB));
	new_pcb->pid=pid_counter++;
	new_pcb->pt0_virt=Get_Page_table(&(new_pcb->pt0_phy));
	pt=new_pcb->pt0_virt;
	...
	if((res=ContextSwitch(MySwitchFunc, &init_pcb->ctxp, (void *)init_pcb, (void *)init_pcb))==-1)
		TracePrintf(0,"\n Error while context switching");
	p=KERNEL_STACK_BASE;
	while(p<KERNEL_STACK_LIMIT)
	{	
		pt[p/PAGESIZE].pfn = GetPage();
		TracePrintf(0,"value of pfn is %d",pt[p/PAGESIZE].pfn); 
		p+=PAGESIZE;
	
	}
	
	//if((res=ContextSwitch(MySwitchFunc, &new_pcb->ctxp, (void *)new_pcb, (void *)init_pcb))==-1)
	//	TracePrintf(0,"\n Error while context switching");
...
SavedContext *MySwitchFunc(SavedContext *ctxp, void *p1, void *p2)
{		long j=KERNEL_STACK_BASE;
		int i=MEM_INVALID_PAGES,index1[4],index2[4],k1=0,k2=0;
		struct pte *pt01,*pt02;
		TracePrintf(0,"Entering MySwitchFunc\n");
		WriteRegister(REG_TLB_FLUSH,TLB_FLUSH_0);
		
		pt01=((struct PCB*)p1)->pt0_virt;
		pt02=((struct PCB*)p2)->pt0_virt;
		if(pt01!=pt02)
		{
		while(j<KERNEL_STACK_LIMIT)
		{	while(pt1[i++].valid==1);
			pt1[i].pfn = pt02[j/PAGESIZE].pfn;
			pt1[i].uprot=0;
		...
		for(i=0;i<4;i++)
		{	//memcpy(index2[i]*PAGESIZE,index1[i]*PAGESIZE,PAGESIZE);
			//Traceprintf(0,"copied %d chars\n",i);
		}		
		/*for(i=0;i<4;i++)
		{	pt1[index1[i]].valid=0;
			pt1[index2[i]].valid=0;
		}*/
...
TracePrintf(0,"LoadProgram : reached 6\n");
	...
	pt=(struct pte *)ReadRegister(REG_PTR0);
	 for(i=MEM_INVALID_SIZE/PAGESIZE;i < PAGE_TABLE_LEN;i++)
...
TracePrintf(0,"LoadProgram : reached 7\n");
	...
	*/
    ...
    // Region 0 page table unused (and thus invalid)
	 pt0=proc_pcb->pt0_virt;
	for(i=0;i<MEM_INVALID_SIZE/PAGESIZE;i++)
		pt0[i].valid = 0;
    ...
    //page table, initialize each PTE:
		for(i=MEM_INVALID_SIZE/PAGESIZE;i<text_npg+MEM_INVALID_SIZE/PAGESIZE;i++)
		{   pt0[i].valid = 1;
            pt0[i].kprot = PROT_READ | PROT_WRITE;
...
long Get_Page_table(long *pt_phy)
{
	...
	{	one_pgt_pages=DOWN_TO_PAGE(max_allocated);
		...
		pt1[index].pfn = pfn_head/PAGESIZE;
		pt1[index].uprot=0;
		pt1[index].kprot=PROT_READ|PROT_WRITE;
		pt1[index].valid=1;
		...
		WriteRegister(REG_TLB_FLUSH,(RCS421RegVal)(index+PAGE_TABLE_LEN)*PAGESIZE);
		pfn_head=*(int *)(long)((index+PAGE_TABLE_LEN)*PAGESIZE);
		TracePrintf(0,"value of pfn_head is %d\n",pfn_head);
		*pt_phy=pt1[index].pfn*PAGESIZE;
		...
		i=MEM_INVALID_PAGES;
		while(pt1[i++].valid!=0);
		pt1[i].pfn = (long)temp/PAGESIZE;
		pt1[i].uprot=0;
		pt1[i].kprot=PROT_READ|PROT_WRITE;
		pt1[i].valid=1;
		j=i*PAGESIZE;
		j=malloc(sizeof(struct pte));

		((struct pte *)j)->valid=1;
		((struct pte *)j)->pfn=-1;
		pt1[index].valid=0;
		WriteRegister(REG_TLB_FLUSH,(RCS421RegVal)(i+PAGE_TABLE_LEN)*PAGESIZE);
...
void ReturnPage(struct pte *pt,int index)
{
	TracePrintf(0,"Entering Return_page\n");
	*(int *)(long)(index*PAGESIZE+VMEM_1_BASE)=pfn_head;
	pfn_head=pt[index].pfn*PAGESIZE;
	TracePrintf(0,"value of pfn_head is %d\n",pfn_head);
