...
void KernelStart(ExceptionStackFrame *frame, unsigned int pmem_size, void *org_brk, char **cmd_args)
{
	...
	*(int *)(long)(addr)=-1;
	...
	TracePrintf(0,"\nvalue of pid is %d",this_pcb->pid);
	...
	idle_pcb->pt0_virt=(struct pte *)Get_Page_table(&(idle_pcb->pt0_phy));
	idle_pcb->pid_parent=init_pcb->pid;

	if((res=ContextSwitch(MySwitchFunc, &idle_pcb->ctxp, (void *)this_pcb, (void *)idle_pcb))==-1)
		TracePrintf(0,"\n Error while context switching");	
	WriteRegister(REG_PTR0,(RCS421RegVal)pt0);
	active_pcb=this_pcb;

	if((res=ContextSwitch(MySwitchFunc, &this_pcb->ctxp, (void *)this_pcb, (void *)idle_pcb))==-1)
		TracePrintf(0,"\n Error while context switching");
	...
	//ReturnPage((long)this_pcb->pt0_phy,pt0+PAGE_TABLE_LEN)*PAGESIZE);
...
int kfork(ExceptionStackFrame *e){
	...
	old_pcb->count_child++;
	new_pcb->pid_parent=old_pcb->pid;
      pt1[i].valid=0;
	e->regs[0]=new_pcb->pid;
	if((ContextSwitch(MySwitchFunc, &new_pcb->ctxp, (void *)old_pcb, (void *)new_pcb))==-1)
        TracePrintf(0,"\n Error while context switching");
      TracePrintf(0,"fork(): pid= %d\n",active_pcb->pid);
	TracePrintf(0,"Exiting fork()\n");
	return (int)active_pcb;
...
void kexit(int status)
{
	...
	WriteRegister(REG_PTR0,(RCS421RegVal)old_pcb->pt0_phy);
	WriteRegister(REG_TLB_FLUSH,TLB_FLUSH_0);
	for (i=MEM_INVALID_PAGES+1;i< KERNEL_STACK_BASE/PAGESIZE;i++)
	{
		if(pt0[i].valid==1)
		{	
	...
	if(active_pcb!=NULL)
	{	TracePrintf(0,"ENtered in if\n");
		if((ContextSwitch(MySwitchFunc, &active_pcb->ctxp, (void *)old_pcb, (void *)active_pcb))==-1)
      		TracePrintf(0,"\n Error while context switching\n");
	}
	else
	{	TracePrintf(0,"ENtered in if else\n");
		if((ContextSwitch(MySwitchFunc, &idle_pcb->ctxp, (void *)old_pcb, (void *)idle_pcb))==-1)
        		TracePrintf(0,"\n Error while context switching");
	}
	WriteRegister(REG_PTR0,(RCS421RegVal)old_pcb->pt0_phy);
	WriteRegister(REG_TLB_FLUSH,TLB_FLUSH_0);
	...
	TracePrintf(0,"In kexit() reached 0 val of phy add is %d",active_pcb->pt0_phy);

	WriteRegister(REG_PTR0,(RCS421RegVal)active_pcb->pt0_phy);

	WriteRegister(REG_TLB_FLUSH,TLB_FLUSH_0);
	TracePrintf(0,"In kexit() reached 1");
	active_pcb->exit_status=status;
