...
long Get_Page_table(long *pt_phy)
{
	...
	{	one_pgt_pages=DOWN_TO_PAGE(max_allocated);
		...
		TracePrintf(0,"Value of ((index+PAGE_TABLE_LEN)*PAGESIZE) is %d",*(int *)(long)((index+PAGE_TABLE_LEN)*PAGESIZE)); 
		...
		*pt_phy=pt1[index].pfn*PAGESIZE;
		...
		index=MEM_INVALID_PAGES+1;
		while(pt1[index++].valid!=0);
		pt1[index].pfn = (long)temp/PAGESIZE;
		pt1[index].uprot=0;
		pt1[index].kprot=PROT_READ|PROT_WRITE;
		pt1[index].valid=1;
		i=index*PAGESIZE;
		i=malloc(sizeof(struct pte));

		((struct pte *)i)->valid=1;
		((struct pte *)i)->pfn=-1;
		pt1[index].valid=0;
...
{
	*(int *)(long)(index*PAGESIZE)=pfn_head;
