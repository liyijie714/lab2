...
void KernelStart(ExceptionStackFrame *frame, unsigned int pmem_size, void *org_brk, char **cmd_args)
{
	...
	{	
		...
		{	//WriteRegister(REG_TLB_FLUSH,index*PAGESIZE);
			*(int *)(long)(addr*PAGESIZE)=(i*PAGESIZE);
				//pt1[index].pfn = i;
				addr=i;
			
		}
	}
	*(int *)(long)(addr*PAGESIZE)=-1;
	//pt1[index].pfn = MEM_INVALID_PAGES;
	TracePrintf(0,"Reached before while\n");
	addr=pfn_head;
	i=*(int *)(long)(addr*PAGESIZE);
	/*while(i!=-1)
	{	TracePrintf(0,"->%x\n",i);
...
int SetKernelBrk(void *addr)
{
	...
	else
	{
		...
		{	
			...
			pt1[index].valid=1;
			pfn_head=(*(int *)(long)(mod_brk))/PAGESIZE;
		}
...
//	pt0=(struct pte *)ReadRegister(REG_PTR0);
	...
	//pt0=Get_Page_table();
	 ...
	 {	if((i*PAGESIZE >= USER_STACK_LIMIT) && (i*PAGESIZE <= KERNEL_STACK_LIMIT))
			continue;
		if (pt0[i].valid ==1)
		{	*(int *)(long)(i*PAGESIZE)=pfn_head*PAGESIZE;
